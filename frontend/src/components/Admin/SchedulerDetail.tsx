...
const SchedulerDetail = () => {
  ...
  const promoteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["scheduler-unsaved", schedulerId] })
  ...
  const deleteUnsavedMutation = useMutation({
    mutationFn: (matchId: string) => SchedulersService.deleteUnsavedMatch({ matchId }),
    onSuccess: () => {
  ...
  const runNowMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["scheduler-activities", schedulerId] })
    },
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/admin/schedulers" })}>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Detailed activity and pending matches for this scheduler.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{scheduler.interval_minutes} min</div>
            <p className="text-xs text-muted-foreground mt-1">Next run based on frequency</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1 truncate italic">{scheduler.last_run_status || "Never run"}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="activities" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>History of scheduler executions and their outcomes.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="unsaved" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Matches that were fetched but didn't meet strict validation (e.g. only one club in season).</CardDescription>
            </CardHeader>
            <CardContent>
               <div className="divide-y border rounded-md">
                {allUnsaved.length === 0 ? (
      ...
      {selectedActivity && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setSelectedActivity(null)}>
              <Card className="max-w-2xl w-full max-h-[80vh] overflow-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <CardTitle>Run Details</CardTitle>
                        <Button variant="ghost" size="icon" onClick={() => setSelectedActivity(null)}>&times;</Button>
                    </div>
                    <CardDescription>
                        Started: {formatDateTime(selectedActivity.started_at)}
                        {selectedActivity.finished_at && ` | Finished: ${formatDateTime(selectedActivity.finished_at)}`}
                    </CardDescription>
                </CardHeader>
                <CardContent>
