...
const route = getRouteApi("/_layout/admin/clubs/$clubId")
...
const ClubDetail = () => {
  ...
  const { data: club, status: clubStatus } = useQuery({
    queryKey: ["club", clubId],
    queryFn: () => ClubsService.readClub({ id: clubId }),
  })

  const { data: stats, status: statsStatus } = useQuery({
    queryKey: ["club-stats", clubId],
    queryFn: () => ClubsService.readClubStats({ id: clubId }),
  })
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-extrabold tracking-tight">Club Profile</h1>
      </div>
